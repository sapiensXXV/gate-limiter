...
func NewDefaultRedisClient(config *settings.RedisClientConfig) *DefaultRedisClient {
...
func (d *DefaultRedisClient) RemoveOldEntries(key string, cutoff time.Time) error {
	...
	return d.client.ZRemRangeByScore(d.ctx, key, "0", strconv.FormatFloat(score, 'f', -1, 64)).Err()
}
...
func (d *DefaultRedisClient) AddToSortedSet(key, member string, t time.Time) error {
	...
	return d.client.ZAdd(d.ctx, key, *z).Err()
}
...
func (d *DefaultRedisClient) RemoveOldEntry(key string, before time.Time) error {
	...
	return d.client.ZRemRangeByScore(d.ctx, key, "0", fmt.Sprintf("%f", score)).Err()
}
...
func (d *DefaultRedisClient) ZCount(key string, min string, max string) (int, error) {
	cnt, err := d.client.ZCount(d.ctx, key, min, max).Result()
	return int(cnt), err
}
